...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $tt = DB::table('problem')
            ->select(DB::raw('group_concat(name) as name, solution, notes, id'))
            ->whereIn('code', $arrCode)
            ->groupByRaw('name, solution, notes, id')
            ->get();
        // dd($tt);
        
        $percent = round($densitas_baru[$codes[0]] * 100, 2);

        $gejala = DB::table('evidence')
                    ->whereIn('id', $request->evidence)
                    ->get();
        ...
        $nama = array(
            ...
            'problem' => json_encode($id_problem)       
        );
        
        DB::table('pasien')->insert($nama);
        
        // dd($rr);

    	return view('hasildiagnosa', compact('percent', 'tt', 'gejala','nama'));
    }
}
